...
mod1 <- function(B,y,mu0,gamma0,nu0,s20){
  ...
  #almacenamiento
  #THETA <- matrix(data = NA, nrow = ceiling((B-1000))/10 ,ncol = 2)
  #LL <- matrix(data = NA, nrow = ceiling((B-1000))/10 ,ncol = 1)
  ...
  for (b in 1:B) {
    ...
    theta <- rnorm(n = 1, mean = (n*ybar/sig2 + mu0/gamma0)*vtheta, sd = sqrt(vtheta))
...
tictoc::tic()
set.seed(2023)
chain1 <- mod1(B = 10000, y = y,mu0 = mu0,gamma0 = gamma0, nu0 = nu0, s20 = s20)
tictoc::toc()
```
```{r}

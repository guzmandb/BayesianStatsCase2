...
```{r}
library(readxl)
pob_mon2018 <- read_excel("pobreza monetaria.xls", sheet = 2, range="A16:P40",col_names = TRUE)

pob_mon2018 <- pob_mon2018[,c(1,16)]
colnames(pob_mon2018) <- c("Departamento","2018")


```
```{r}
cod <- datos%>%
  select(ESTU_DEPTO_RESIDE,ESTU_COD_RESIDE_DEPTO)%>%
  distinct()
cod <- na.omit(cod)
```

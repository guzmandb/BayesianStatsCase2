...
Mapa_mpios <- punt_muestrales_mpios %>%
  left_join(estadisticas_edu,by = c("COLE_COD_MCPIO_UBICACION"="CÓDIGO_MUNICIPIO"))%>%
  as.data.frame()
...
```{r}
calculate_waic= function(log_lik){
  waic_result=loo::waic(log_lik)
  return(waic_result)
}

log_lik_model1=as.matrix(chain1$LL)
waic_model1=calculate_waic(log_lik_model1)

log_lik_model2=as.matrix(chain2$LL)
waic_model2=calculate_waic(log_lik_model2)
print(waic_model2)

log_lik_model3=as.matrix(chain3$LL)
waic_model3=calculate_waic(log_lik_model3)
print(waic_model3)

log_lik_model4=as.matrix(chain4$LL)
waic_model4=calculate_waic(log_lik_model4)
print(waic_model4)

log_lik_model5=as.matrix(chain5$LL)
waic_model5=calculate_waic(log_lik_model5)
print(waic_model5)

(waic1libreria=waic_model1$estimates[3])
(waic2libreria=waic_model2$estimates[3])
(waic3libreria=waic_model3$estimates[3])
(waic4libreria=waic_model4$estimates[3])
(waic5libreria=waic_model5$estimates[3])
WAICsLibreria <- c(waic1libreria,waic2libreria,waic3libreria,waic4libreria,waic5libreria)
WAICsLibreria
(which.min(WAICsLibreria))
```


```{r Punto 5 WAIC}
n <- length(datos$PUNT_GLOBAL)
m <- length(unique(datos$COLE_COD_DEPTO_UBICACION))
y <- datos$PUNT_GLOBAL
...
tictoc::tic()
set.seed(2023)
#foreach(i = 1:n, .combine = "+", .packages = c("boot")) %dopar% {
for (i in 1:n){
  # lppd
  tmp1 <- dnorm(x = y[i], mean = chain2$THETA[,g[i]], sd = sqrt(chain2$THETA$sig2))
...
for (i in 1:n) {
  # lppd
  tmp1 <- dnorm(x = y[i], mean = chain3$THETA[,g[i]], sd = sqrt(chain3$THETA[,m+g[i]]))
  lppd_m3 <- lppd_m3 + log(mean(tmp1))
  # pWAIC
  tmp2 <- dnorm(x = y[i], mean = chain3$THETA[,g[i]], sd = sqrt(chain3$THETA[,m+g[i]]), log = T)
...
```{r}
calcular_WAIC <- function(y, muestras, log_verosimilitudes) {
  S <- nrow(chain2$THETA)
  n <- length(y)
  
  # Calcular el término de log-verosimilitud puntual
  lppd <- sum(log(rowMeans(exp(chain2$LL))))
  
  # Calcular el término de varianza efectiva
  p_eff <- sum(apply(chain2$LL, 1, var))
...
# Calcula el WAIC
waic <- calcular_WAIC(y = y, muestras = chain2$THETA, log_verosimilitudes = chain2$LL)
print(waic)
```
...
for (i in 1:n) {
  # lppd
  tmp1 <- dnorm(x = y[i], mean = chain2$THETA[,g2[i]], sd = sqrt(chain2$THETA$sig2))
  lppd_m1 <- lppd_m1 + log(mean(tmp1))
  # pWAIC
  tmp2 <- dnorm(x = y[i], mean = chain2$THETA[,g2[i]], sd = sqrt(chain2$THETA$sig2),log = T)
...
m <- length(unique(datos$COLE_COD_DEPTO_UBICACION))
y <- datos$PUNT_GLOBAL
f <- length(unique(datos$COLE_COD_MCPIO_UBICACION))
...
for (i in 1:n) {
  # lppd
  tmp1 <- dnorm(x = y[i], mean = chain1$THETA[,1], sd = sqrt(chain1$THETA[,2]))
  lppd_m0 <- lppd_m0 + log(mean(tmp1))
  # pWAIC
  tmp2 <- dnorm(x = y[i], mean = chain1$THETA[,1], sd = sqrt(chain1$THETA[,2]), log = T)
  pWAIC_m0 <- pWAIC_m0 + 2*(log(mean(tmp1)) - mean(tmp2))
}
waic_m1 <- -2*lppd_m0 + 2*pWAIC_m0
...
waic_m3 <- -2*lppd_m2 + 2*pWAIC_m2


###WAIC 4
Ym <- vector(mode = "list", length = f)
r <- rep(NA, n)
for (j in 1:f) {
  idx2 <- datos$COLE_COD_MCPIO_UBICACION == unique(datos$COLE_COD_MCPIO_UBICACION)[j]
  r[idx2] <- j
  Ym[[j]] <- y[idx2]
}
...
for (i in 1:n) {
  # lppd
  tmp1 <- dnorm(x = y[i], mean = chain4$ZETA[,r[i]], sd = sqrt(chain4$OTROS$k2))
  lppd_m3 <- lppd_m3 + log(mean(tmp1))
  # pWAIC
  tmp2 <- dnorm(x = y[i], mean = chain4$ZETA[,r[i]], sd = sqrt(chain4$OTROS$k2),log = T)
  pWAIC_m3 <- pWAIC_m3 + 2*(log(mean(tmp1)) - mean(tmp2))
}
waic_m4 <- -2*lppd_m3 + 2*pWAIC_m3
...
for (i in 1:n) {
  # lppd
  tmp1 <- dnorm(x = y[i], mean = chain5$ZETA[,r[i]], sd = sqrt(chain5$OTROS$k2))
  lppd_m4 <- lppd_m4 + log(mean(tmp1))
  # pWAIC
  tmp2 <- dnorm(x = y[i], mean = chain5$ZETA[,r[i]], sd = sqrt(chain5$OTROS$k2),log = T)
  pWAIC_m4 <- pWAIC_m4 + 2*(log(mean(tmp1)) - mean(tmp2))
}
waic_m5 <- -2*lppd_m4 + 2*pWAIC_m4
...
set.seed(2023)
for (i in 1:10000) {
  departamentos <- t(chain5$THETA[i,]) # Obtiene la fila i
...
ybayes <- as.vector(colMeans(chain5$THETA))
orden <- order(ybayes,decreasing = T)
# se organizan las observaciones de acuerdo a la partición verdadera
set.seed(2023)
kbayes <- kmeans(ybayes, centers = clusters)
xi = kbayes$cluster
indices <- order(xi)
indices <- orden
A <- A[indices,indices]
...
nombres <- estadisticos$nombre
str(nombres)
pos <- seq(1:32)
nombres <- cbind(nombres,pos)
nombres <- as.data.frame(nombres)

nombres$pos <- as.integer(nombres$pos)


TAGS <- indices %>%left_join(nombres, by = c("indices"="pos"))
str(TAGS)
colnames(A) <- TAGS$nombres
rownames(A) <- TAGS$nombres

# visualización de la matriz de incidencia
  par(mar = c(4.5,2.75,0.5,0.5), mgp = c(1.7,0.7,0))
  corrplot::corrplot(corr = A, is.corr = FALSE, addgrid.col = NA, method = "color", tl.pos = "lt",title = "Matriz de Incidencia para Departamentos",mar = c(0,0,1,0),tl.cex = 0.59,tl.col = "black")
```
```{r}
# k means para cada iteracion
...
```{r}
Mapa8 <- cbind(Mapa,xi)
Mapa8$xi <- as.character(Mapa8$xi)
...
mapdeptos8 <- deptoshp %>%
  left_join(Mapa8,by=c("DPTO_CCDGO"="cod_dep")) 
...
box8 <- st_bbox(mapdeptos8)

plot8 <- ggplot() +
  geom_sf(data = mundocol) +
  geom_sf(data = mapdeptos8, aes(fill = xi), col = "darkgray", linetype = "solid") +
  coord_sf(xlim = c(box8$xmin, box8$xmax), ylim = c(box8$ymin, box8$ymax), expand = FALSE) +
  labs(x = "Longitud", y = "Latitud", title = "Clustering por departamentos en Colombia \nsegun el puntaje medio en la prueba SABER 2022-II", fill = "Clusters") +
  scale_fill_manual(values = c("#FF7F00","#32FF00","#FFFF32","#654CFF","#E51932"))
...
ICIP2018 <- apply(predicciones, MARGIN = 2, function(x) quantile(x, c(0.025, 0.975)))
predIP2018 <- round(predIP2018,digits = 4)
ICIP2018 <- round(ICIP2018,digits = 4)

punto_9 <- rbind(predIP2018,ICIP2018)
...
nombresNA <- deptosNA %>%
  left_join(datos,by=c("cod_dep"="COLE_COD_DEPTO_UBICACION"))%>%
  select(cod_dep,COLE_DEPTO_UBICACION)%>%
  as.data.frame()

nombresNA <- nombresNA %>% distinct(cod_dep, .keep_all = TRUE)

## Media posterior, Intervalos de confianza.
(punto_9 <- `colnames<-`(punto_9, nombresNA$COLE_DEPTO_UBICACION))

nombresNA$IP2018 <- as.vector(punto_9[1,])

# Obtén el índice de ordenación de las medias posteriores
orden <- order(punto_9["predIP2018", ],decreasing = T)

# Reordena el dataframe punto_9 según el índice de orden
punto_9_ordenado <- punto_9[, orden]

# Muestra el dataframe ordenado
print(punto_9_ordenado)
```
```{r}
library(xtable)
xtable(punto_9_ordenado, file = "sample_table.tex")
```
...
mapdeptosNA <- deptoshp %>%
  left_join(nombresNA,by=c("DPTO_CCDGO"="cod_dep")) 

mundoshp <- st_read("C:/Bayesiana/Caso2/misc/admin00.shp",quiet=TRUE)
mundocol <- mundoshp %>% 
            filter(CNTRY_NAME %in% c("Peru","Brazil","Venezuela","Ecuador","Panama"))

box5 <- st_bbox(mapdeptosNA)

plot5 <- ggplot() +
  geom_sf(data = mundocol) +
  geom_sf(data = mapdeptosNA, aes(fill = IP2018), col = "darkgray", linetype = "solid") +
  coord_sf(xlim = c(box5$xmin, box5$xmax), ylim = c(box5$ymin, box5$ymax), expand = FALSE) +
  geom_sf_text(data = mapdeptosNA, aes(label = COLE_DEPTO_UBICACION), col = "black",
               fontface = "bold", size = 2, fun.geometry = function(x) sf::st_centroid(x)) +
  labs(x = "Longitud", y = "Latitud", title = "Predicción de Índice de pobreza \nen el 2018 para los departamento \nno medidos por el DANE", fill = "índice de pobreza") +
  scale_fill_gradient(low = "white", high = "#FFD700", n.breaks = 5)
...
# k means para cada iteracion
tictoc::tic()
  nj <- length(unique(datos$COLE_COD_MCPIO_UBICACION))
  
  # Número de clústeres deseados
  clusters_mpios <- 8
  
  # Inicializa una lista para almacenar los resultados
  kmedias_mpios <- matrix(NA,nrow=10000,ncol=nj)
  # Realiza K-Means en cada fila de la matriz de datos
  set.seed(2023)
  for (i in 1:10000) {
    municipios <- t(chain5$ZETA[i,]) 
    clusters_kmedias_mpios <- kmeans(municipios, centers = clusters_mpios)  
    kmedias_mpios[i,] <- clusters_kmedias_mpios$cluster  
  }

  
A_mpios <- matrix(data = 0, nrow = nj, ncol = nj)
B <- nrow(kmedias_mpios)
for (b in 1:B) {
  for (i in 1:(nj-1)) {
    for (j in (i+1):nj) {
      if (kmedias_mpios[b,i] == kmedias_mpios[b,j]) {
        A_mpios[i,j] <- A_mpios[i,j] + 1/B
      } 
    }
  }
}
...
A_mpios <- A_mpios + t(A_mpios)
diag(A_mpios) <- 1



yzeta <- as.vector(colMeans(chain5$ZETA))
# se organizan las observaciones de acuerdo a la partición verdadera
set.seed(2023)
kzeta <- kmeans(yzeta, centers = clusters_mpios)
xi_zeta = kzeta$cluster
orden_z = order(yzeta,decreasing = T)
indices_z <- orden_z
A_mpios <- A_mpios[indices_z,indices_z]

indices_z <- as.data.frame(indices_z)
...
nombres_mpios <- estadisticas_mpios$nombre 
pos_mpios <- seq(1:nj)
nombres_mpios <- cbind(nombres_mpios,pos_mpios)
nombres_mpios <- as.data.frame(nombres_mpios)

nombres_mpios$pos_mpios <- as.integer(nombres_mpios$pos_mpios)



TAGS_mpios <- indices_z %>%left_join(nombres_mpios, by = c("indices_z"="pos_mpios"))
str(TAGS_mpios)
colnames(A_mpios) <- TAGS_mpios$nombres_mpios
rownames(A_mpios) <- TAGS_mpios$nombres_mpios

# visualización de la matriz de incidencia
par(mar = c(6.5, 2.75, 0.5, 0.5), mgp = c(1.7, 0.7, 0))
corrplot::corrplot(corr = A_mpios, is.corr = FALSE, addgrid.col = NA, method = "color", tl.pos = "n", title = "Matriz de Incidencia para municipios",mar = c(0,0,1,0))
tictoc::toc()


```

### matriz incidencia no ordenada por ranking 
```{r para clusters municipios no ordenados}
# k means para cada iteracion
...
Mapa10 <- cbind(Mapa_mpios,xi_zeta)
Mapa10$xi_zeta <- as.character(Mapa10$xi_zeta)
...
mapmpios <- mpioshp %>%
  left_join(Mapa10,by=c("MPIO_COD"="COLE_COD_MCPIO_UBICACION")) 
...
box10 <- st_bbox(mapmpios)
   ggplot() +
  geom_sf(data = mundocol) +
  geom_sf(data = mapmpios, aes(fill = xi_zeta), col = "gray", linetype = "solid", size=0.001) +
  coord_sf(xlim = c(box10$xmin, box10$xmax), ylim = c(box10$ymin, box10$ymax), expand = FALSE) +
  labs(x = "Longitud", y = "Latitud", title = "Clustering por puntaje medio en la prueba SABER \npor municipios en Colombia", fill = "Cluster") +
  scale_fill_manual(values = c("#C86925","#FDF56C","#FC9576","#5D57AF","#B2CE31","#FF2027","#B1D6C5","#AB8323"))

```

## Punto 11
```{r}
Mapa_mpiosNA <- Mapa_mpios %>% 
  filter(is.na(COBERTURA_NETA_SECUNDARIA)) %>%
  as.data.frame()

valores_na <- which(is.na(Mapa_mpios$COBERTURA_NETA_SECUNDARIA))
valores_na

regresion_mpios <- Mapa_mpios %>%
  select(COLE_COD_MCPIO_UBICACION, MUNICIPIO, COBERTURA_NETA_SECUNDARIA) %>%
as.data.frame()

predicciones_mpios <- matrix(data = NA, nrow = 10000, ncol=2)

for (b in 1:10000) {
  media <- unlist(chain5$ZETA[b,]) 
  regresion_mpios$media <- media
  modelo <- lm(COBERTURA_NETA_SECUNDARIA ~ media, data = regresion_mpios)
  datos_na <- media[c(582,1098)]
  new_data <- data.frame(media = datos_na)
  a = predict(modelo,newdata = new_data)
  predicciones_mpios[b, ] <- a
}
...
```{r}
predCNS <- colMeans(predicciones_mpios)
ICCNS <- apply(predicciones_mpios, MARGIN = 2, function(x) quantile(x, c(0.025, 0.975)))
predCNS <- round(predCNS,digits = 4)
ICCNS <- round(ICCNS,digits = 4)

(punto_11 <- rbind(predCNS,ICCNS))
```
...
estadisticas_mpios <- datos %>%
  ...
  summarise(codigo = unique(COLE_COD_MCPIO_UBICACION),
            ...
            njk = n(),
            ybjk = mean(PUNT_GLOBAL),
            minjbk = min(PUNT_GLOBAL),
            maxjbk = max(PUNT_GLOBAL),
            ICjbk = quantile(PUNT_GLOBAL, c(0.025,0.975)),
            medianjbk = median(PUNT_GLOBAL),
            desv_estjbk = sd(PUNT_GLOBAL),
            s2jk = var(PUNT_GLOBAL),
            IQRjbk = IQR(PUNT_GLOBAL))%>%
  arrange(codigo) %>% 
  as.data.frame()
...
ybjk <- estadisticas_mpios$ybjk
minjbk <- estadisticas_mpios$minjbk
maxjbk <- estadisticas_mpios$maxjbk
ICjbk <- estadisticas_mpios$ICjbk
medianjnk <- estadisticas_mpios$medianjbk
desv_estjbk <- estadisticas_mpios$desv_estjbk
IQRjbk <- estadisticas_mpios$IQRjbk

etiquetas_njk <- rep(1:length(njk),times = njk)

intervalo_confianza_quantile <- function(x) {
  cuantiles <- quantile(x, c(0.025, 0.975))
  return(cuantiles)
}



minimos <- matrix(data = NA, nrow = 10000, ncol=length(njk))
maximos <- matrix(data = NA, nrow = 10000, ncol=length(njk))
ICs <- matrix(data = NA, nrow = 10000, ncol=2*length(njk))
medias <- matrix(data = NA, nrow = 10000, ncol=length(njk))
mediana <- matrix(data = NA, nrow = 10000, ncol=length(njk))
desvestandar <- matrix(data = NA, nrow = 10000, ncol=length(njk))
IQRs <- matrix(data = NA, nrow = 10000, ncol=length(njk))


set.seed(2023)
for (i in 1:10000) {
  zetas <- as.numeric(chain5$ZETA[i,])
  k2 <- chain5$OTROS[i,1]
  ysimulados <- rnorm(rep(njk,njk), mean = rep(zetas,njk),sd = sqrt(k2))
  minimos[i,] <- tapply(ysimulados, etiquetas_njk,min)
  maximos[i,] <- tapply(ysimulados, etiquetas_njk,max)
  medias[i,] <- tapply(ysimulados, etiquetas_njk,mean)
  ICs[i,] <- unlist(tapply(ysimulados, etiquetas_njk, intervalo_confianza_quantile))
  mediana[i,] <- tapply(ysimulados, etiquetas_njk,median)
  desvestandar[i,] <- tapply(ysimulados, etiquetas_njk,sd)
  IQRs[i,] <- tapply(ysimulados,etiquetas_njk, IQR)
}

PPPmedia <- colMeans(medias<ybjk)
PPPmin <- colMeans(minimos < minjbk)
PPPmax <- colMeans(maximos < maxjbk)
PPPmediana <- colMeans(mediana < medianjnk)
PPPsd <- colMeans(desvestandar < desv_estjbk)
PPPic <- colMeans(ICs < ICjbk  )
PPPiqr <- colMeans(IQRs < IQRjbk)
...
dfPPP <- data.frame(
  ...
  PPPmin = PPPmin,
  PPPmax = PPPmax,
  PPPmediana = PPPmediana,
  PPPsd = PPPsd,
  PPPiqr = PPPiqr
)

par(mar = c(5, 4, 4, 2) + 0.5)
boxplot(dfPPP, main = "Boxplots de los valores ppp para \ndiferentes estadísticos de prueba", ylab = "ppp",xlab = "Estadísticos de prueba", col = "lightblue", border = "black",names = c("media","min","max","mediana","sd","IQR"), outline = FALSE)

for (i in 1:ncol(dfPPP)) {
  x_pos <- jitter(rep(i, length(dfPPP[[i]]), factor = 0.4))  # Ajusta el factor de dispersión según tu preferencia
  points(x_pos, dfPPP[[i]], col = "#4D4D4D", pch = 16, cex = 0.5)
}
